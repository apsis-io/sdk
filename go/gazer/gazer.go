...
// Package gazer is the Gazer identity vocabulary: the subject convention a
// device's trail leaf carries, and the parser and builder that must agree about
// it forever.
//
// ***APACHE-2.0 WHILE THE SEAM IT CAME FROM IS BUSL, AND THE SPLIT IS THE POINT.***
// This left magicseam on 2026-09-02 because periapsis-CE is GPLv3 and depended
// on exactly these four symbols; BUSL carries production-use conditions and
// GPLv3 section 10 forbids imposing further restrictions downstream, so a CE
// that needed magicseam could not honour its own licence. Nothing here touches
// the seam - it is string handling over a naming convention, stdlib only - so it
// is the half that can be permissive, and the SDK already holds the principle
// that a consumer-facing vocabulary should not restrict its consumers.
//
...
var ErrUnattested = errors.New("gazer: caller presented no verified identity")

// ErrNotAGazer is returned when a verified identity exists but does not name a
// Gazer.
var ErrNotAGazer = errors.New("gazer: verified identity is not a Gazer principal")
